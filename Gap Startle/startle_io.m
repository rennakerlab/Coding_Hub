...
intensities = [80:5:110];                                                   %set the intensities to be tested as a range using min:max
reps = 10;                                                                  %set how many times you want to test each intensity
pre_reps = 6;
...
rp2_RCX_filename = [rcxpath 'Gap_In_Loaded_Background_Sound.rcx']; 	%Name of the RP2 RCX file.
...
cueprob = 0;      %Probability of any trial being a cued trial.
...
startleint = 115;   %Startler intensity, in dB.
...
center_freqs = 0';         %The user can pick between these center frequencies.
...
noises(2:size(noises,1),2) = 1/3; 	%The second column will be bandwidth (in kHz).
noises(2:size(noises,1),3) = 2;    	%The third column will be the filter order.
noisetags{1} = 'Broadband Noise';           %The first option will be broadband noise.
for i = 2:size(noises,1)                  	%Step through all other noises.
...
filename = ['STARTLEint_' num2str(i) '_' ratname '_' num2str(daycodestr,'%03d')];    %Save the root filename.
...
for s = soundorder
    ...
    %Set the startler intensity.
        ...
        volt = exp((randomizedints(i) - a(1))/a(2));        %Find the required voltage to make the startler noise intensity.
        startleint = randomizedints(i);                     %set the startleint for recording in the .startle file
        ...
        if volt > 10                                        %If the voltage of the startler is greater than the RP2 can handle...
            ...
            disp(['The startler intensity will probably be less than the specified intensity of ' num2str(randomizedints(i)) ' dB. Actual intensity is: ', num2str(actualint), 'dB.']);                                              %Warn the user that the startle intensity will won't be correct.
        end
        ...
        rp2.SetTagVal('cue_enable',cueorder(i));           	%Enable/disable the gap.
       ...
       disp(['Rejoice and be mirthful! ', num2str(i), ' startle(s) hath come and gone.'])
       pause(iti);
...
for k = 1:length(intensities)
    ...
    noisecis(k) = simple_ci(cell2mat(startledata(thisintind,4)));
end

iodata_meanci = [startlemeans; startlecis; noisemeans; noisecis];

save([savepath ratname, '_startleint_', datestr(now,'mm-dd-yyyy'), '.mat'], 'iodata_meanci', 'startledata');
figure
errorbar(startlemeans,startlecis)
text2speech('Startle I.O. Assessment Complete');
