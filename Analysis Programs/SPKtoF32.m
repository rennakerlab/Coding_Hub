...
%               value is 'Off'.
%
%   * 'Order' - This option allows an user to export the order of stimulus
%               presentation as an extra parameter in the *.f32 file.
%               Note, however, that this will cause each sweep to be listed
%               as a unique stimulus in the *.f32 file, since each sweep
%               will have a unique parameter set.  Input values for the
%               'Order' property are 'On' or 'Off', with a default value of
%               'On'.
%
    ...
    if length(varargin) <= i                                                %If any input argument doesn't have a matching value...
        ...
        elseif strcmpi(varargin{i},'order')                                 %If we're setting the 'order' property...
            if ~ischar(varargin{i+1}) ||...
                ...
                error('ERROR IN SPKTOF32: The ''Order'' property must set to ''On'' or ''Off''.');	%Show an error.
            else                                                            %Otherwise, if the input value is 'on' or 'off'...
...
while ~feof(spkfid)                                                         %Loop until the end of the file.
    ...
    catch                                                                   %If there was an error reading the last sweep, show the user a warning.
        warning(['Error in reading sweep ' num2str(i) ' for this file,'...
            ' stopping file read at last complete sweep.']);                %#ok<WNTAG>
    end
end
