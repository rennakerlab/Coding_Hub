function tc_compare(varargin)

%
%TC_COMPARE.m - Rennaker Neural Engineering Lab, 2010
%
%   TC_COMPARE has the user select an *.f32 file or an *.LFP file and then 
...
%
%   TC_COMPARE(file,...) creates a TC plot and pooled PSTH/AER for each
%   file specified.  The variable "file" can either be a single string or a
%   cell array of strings containing file names.
%
%   TC_COMPARE(binsize,...) creates the pooled PSTH/AER smoothed with a 
...
if length(varargin) > 2             %If the user entered too many arguments, show an error.
    error('Too many input arguments for TC_COMPARE!  Inputs should be a filename string, or cell array of filename strings, and/or an integer bin size.');
end
for i = 1:length(varargin)
    ...
    elseif isnumeric(temp)                  %If the argument is a number...
        ...
        error(['Input argument #' num2str(i) ' is not recognized in TC_COMPARE!  Inputs should be a filename string, or cell array of filename strings, and/or an integer bin size.']);
    end
end
...
for i = 1:length(files)
    ...
    if isf32                                %If this is f32 data...
        params = vertcat(data(:).stim);   	%Grab the stimulus parameters from the *.f32 formatted data.
    ...
    if length(ints) > 1
        subplot(2,2,4);                             %Plot the pooled PSTH as the bottom 1/3rd of the figure.
    else
    ...
    if isf32                                        %If the file is an *.f32 file...
        if i == 1
            plot(psth,'color','b','linewidth',2); 	%Plot the pooled PSTH as an area plot.
            ylabel('spikerate (spks/s)');               %Label the y-axis.
        else
            plot(psth,'color',[0.5 0 0],'linewidth',2); 	%Plot the pooled PSTH as an area plot.
        end
    else                                            %Otherwsie, if the file is an *.LFP file...
        if i == 1
            plot(aer,'color','b','linewidth',2);        %Plot the AER as a line.
            line([0,sweeplength],[0 0],'color','k','linestyle','--');   %Plot a line to show zero voltage.
            ylabel('evoked response (\muV)');        	%Label the y-axis.
        else
            plot(aer,'color',[0.5 0 0],'linewidth',2);        %Plot the AER as a line.
        end
    end
    if i == 2
        ...
        if ~isf32                         	%If this isn't f32 data...
    ...
    if length(ints) > 1                 %If this is a multi-intensity TC...
        tc = zeros(length(freqs),length(ints));       %Create an empty array to hold excitatory tuning curves.
        if isf32                                        %If this is f32 data...
            for j = 1:length(data)                      %Step through each stimulus...
                tc(data(j).stim(freq_index) == freqs, data(j).stim(int_index) == ints) = ...
                    range(boxsmooth(data(j).psth,binsize));                 %Find the mean spikerate in each excitatory segment.
            end
            ...
            for j = 1:length(data.stim)                 %Step through each stimulus...
                tc(data.param(freq_index).value(j) == freqs, data.param(int_index).value(j) == ints) = ...
                    -range(data.stim(j).aer(1:end-1));	%Find the range of the mean evoked response, kicking out the last sample.
            end
        ...
        subplot(2,2,i); 	%Plot the tuning curve as the top 2/3rds of the figure.        
        temp = boxsmooth(tc(:,:)',smooth_factor);    %Boxsmooth spikerates in the first excitatory segment.
        pad = mean(mean(temp));
        temp = [[temp, repmat(pad,size(temp,1),1)]; repmat(pad,1,size(temp,2)+1)];    %Pad the edges with zeros so it shows the whole curve.
        if i == 1
            difference = temp;
        else
        ...
        if isf32                        %If this is f32 data...
            for j = 1:length(data)      %Step through each stimulus...
                ...
                tc(data(j).stim(freq_index) == freqs, 1) = mean(range(boxsmooth(temp,[1 binsize]),2));              %Find the mean spikerate in each excitatory segment.
                tc(data(j).stim(freq_index) == freqs, 2) = simple_ci(range(boxsmooth(temp,[1 binsize]),2),alpha);	%Calculate confidence intervals for spikerate.
            end
        else                                %Otherwise, if this is LFP data...
            for j = 1:length(data.stim)     %Step through each stimulus...
                temp = 1000000*data.stim(j).lfp;  	%Grab the LFPs for this stimulus.
                tc(data.param(freq_index).value(j) == freqs, 1) = mean(range(temp,2));           %Find the mean LFP range in each excitatory segment.
                tc(data.param(freq_index).value(j) == freqs, 2) = simple_ci(range(temp,2),alpha);    %Calculate confidence intervals for LFP ranges.
            end
        end
        ...
        if i == 1
            errorbar(tc(:,1),tc(:,2),'color','b','linewidth',2);	%Plot the excitatory segment spikerate in red.
        else
            errorbar(tc(:,1),tc(:,2),'color',[0.5 0 0],'linewidth',2);	%Plot the excitatory segment spikerate in red.
        end
        if i == 2
...
subplot(2,2,3); 	%Plot the tuning curve as the top 2/3rds of the figure.        
surf(difference,'edgecolor','none');          %Plot the first excitatory segment as a surface plot.
if isf32                    %If this is f32 data...
    colormap(jet(500));   	%Color the surface with a non-inverted jet.
else                        %Otherwise, if this is LFP data.
    colormap(flipud(jet(500)));	%Color the surface with an inverted jet.
