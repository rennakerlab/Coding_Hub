function quick_psth(varargin)

%
%QUICK_PSTH.m - Rennaker Neural Engineering Lab, 2010
%
%   QUICK_PSTH has the user select an *.f32 file and then creates a pooled 
%   PSTH from those spike times.
%
%   QUICK_PSTH(file,...) creates a pooled PSTH for each file specified.  The 
%   variable "file" can either be a single string or a cell array of 
%   strings containing file names.
%
%   QUICK_PSTH(binsize,...) creates a pooled PSTH smoothing with a longer
...
if length(varargin) > 2             %If the user entered too many arguments, show an error.
    error('Too many input arguments for QUICK_PSTH!  Inputs should be a filename string, or cell array of filename strings, and/or an integer bin size.');
end
for i = 1:length(varargin)
    ...
    elseif isnumeric(temp)                  %If the argument is a number...
        ...
        error(['Input argument #' num2str(i) ' is not recognized in QUICK_PSTH!  Inputs should be a filename string, or cell array of filename strings, and/or an integer bin size.']);
    end
end
...
%     params = horzcat(data.params)';                                         %Concatenate the stimulus parameters into one matrix.
%     params(:,7) = params(:,3) - params(:,6);                                %Subtract the masker onset from the probe onset to find the SOA.
%     [params, j] = sortrows(params,[2,5,7]);                                 %Sort the data first by probe frequency, then by probe intensity, then by SOA.
%     data = data(j);                                                         %Resort the whole data structure by the same indices as the parameters.
...
%     for j = 1:size(stsad,1)                                                 %Step through each row of the STSAD.
%         temp = stsad(j,params(j,6)-34:end);                                 %Grab the sweep starting 35 ms before the masker.
%         stsad(j,1:length(temp)) = temp;                                     %Shift over the recorded sweep to align maskers.
%         stsad(j,length(temp)+1:end) = NaN;                                  %Replace the end of the row with NaNs.
%     end
