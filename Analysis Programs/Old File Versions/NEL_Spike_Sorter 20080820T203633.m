function varargout = NEL_Spike_Sorter(varargin)
...
%   template.
%
%   Last updated July 11, 2008, by Drew Sloan.
...
function NEL_Spike_Sorter_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    elseif exist('C:\Documents and Settings\All Users\Desktop','dir') == 7
        handles.desktop = 'C:\Documents and Settings\All Users\Desktop';
    end
...
function [handles, max_spike_break] = LoadSpikes(handles, hObject, save_option)
    data = SPKFileRead(handles.file);	%Read in the spikeshape data.
    %We'll go through the data structure and pull out all the spikeshapes and spiketimes.
    handles.spikes = [];
    handles.times = [];
    handles.cluster = [];
    handles.numsweeps = 0;
    max_spike_break = [];           %Keep track of the point after which we've got >25,000 spikes.
    for i = 1:length(data.stim)     %Step through by stimulus...
        for j = 1:length(data.stim(i).spikes)   %Step through by repetition...
            handles.spikes = [handles.spikes; data.stim(i).spikes(j).shapes];       %Load up spikeshapes.
            handles.cluster = [handles.cluster; data.stim(i).spikes(j).cluster];	%Load up cluster assignments.
            handles.times = [handles.times; data.stim(i).spikes(j).times];          %Load up spike times.
            %Clear out the spike shapes from the data structure to save space.
            data.stim(i).spikes(j).shapes = [];
            if size(handles.spikes,1) > handles.max_spikes  %If we've got more spikes than we want to keep for plotting...
                if strcmpi(save_option,'Save')
                    if isempty(max_spike_break)
                        max_spike_break = length(handles.cluster) + 1;	%If there's more than 25,000 spikes, we'll trim down the loaded-up spikes after saving.
                    end
                elseif strcmpi(save_option,'Plot')
        ...
        if size(handles.spikes,1) > handles.max_spikes & strcmpi(save_option,'Plot')
            break;
        end
    end
    handles.numspikes = size(handles.spikes,1);     %Find the exact number of spikeshapes we've grabbed.
    handles.spont_delay = double(data.spont_delay);	%Grab the spontaneous delay from the data structure.
    handles.rat = data.rat;                         %Grab the rat name from the data structure.
    handles.sampling_rate = data.sampling_rate;     %Grab the sampling rate from the data structure.
    handles.sweeplength = 1000*min([data.stim(:).sweeplength]);	%Grab the shortest sweeplength from the data structure.
    ...
    handles.min_clus = length(data.stim)*length(data.stim(1).spikes)/2;     %Set a minimum cluster size for SPC.
    clear data;	%Clear the data structure to free up RAM.
...
function handles = PlotClusters(handles, hObject)
    ...
    for c = 0:handles.num_clusters          %Step through by cluster.
        ...
        elseif c ~= 0               %If the cluster exists, but there were no plot spikes in the randomized list...
            waveshapes = [waveshapes; repmat(NaN,1,size(handles.spikes,2))];    %Hold the cluster's place with a line of NaNs.
        end
...
function handles = TemplateSortSpikes(handles, hObject)
    ...
    distances = repmat(NaN,handles.numspikes,handles.num_clusters); %Set up a matrix to receive Euclidian distances.
...
function handles = CalculatePCAs(handles, hObject)
    ...
    temp = repmat(NaN,size(handles.spikes));	%Set up the aligned-spike matrix first for speed.
...
function handles = SaveClusters(handles, hObject)
    ...
    text(0,0,['Sorting all spikes for: ' temp],'interpreter','none','fontweight','bold',...    %Show the "Loading..." message.
        'color','w','fontsize',16,'horizontalalignment','center','verticalalignment','middle');
    drawnow;    %Refresh the axes before running the rest of the code.
    [handles, max_spike_break] = LoadSpikes(handles, hObject, 'Save');	%Load all the spikes for this file.   
    if handles.threshold(2) > 0     %If the threshold is positive, set all spikes below the threshold to noise.
        handles.subthreshold = find(handles.spikes(:,handles.threshold(1)) < handles.threshold(2));
    else    %If the threshold is negative, set all spikes above the threshold to noise.
        handles.subthreshold = find(handles.spikes(:,handles.threshold(1)) > handles.threshold(2));
    ...
    if strcmpi(handles.sort_type,'Template')    %If we're template sorting...
        handles = TemplateSortSpikes(handles, hObject);     %Sort using existing templates.
    elseif strcmpi(handles.sort_type,'PCA')     %If we're PCA sorting...
        handles = CalculatePCAs(handles, hObject);  %Calculate the principal components.
        handles = ClusterByPCA(handles, hObject);   %Cluster using PCA boundaries.
    elseif strcmpi(handles.sort_type,'Window')  %If we're window sorting...
        handles = ClusterByWindow(handles, hObject);    %Cluster using windows.
    ...
    n = 1;                              %Keep track of which spike assignment we're writing to file.
    while ~feof(fid) & n <= handles.numspikes
        i = fread(fid,1,'int16');       %Stimulus index
        if ~isempty(i)
            fseek(fid,4 + 4*numparam,'cof');	%Skip the stimulus index, sweeplength, parameter values, and number of sweeps.
            numsweeps = uint16(fread(fid,1,'uint16'));      %Number of sweeps to follow.
            for j = 1:numsweeps
                fseek(fid,14,'cof');        %Skip the timestamp, trial number, and noise estimate..
                numspikes = uint16(fread(fid,1,'uint32'));	%Number of spikes in this sweep.
                for k = 1:numspikes    %Go through spike by spike and modify cluster assignments.
                    fseek(fid,4,'cof');     %Skip the spike time.
                    fwrite(fid,handles.cluster(n),'uint8');  %WRITE IN THE NEW CLUSTER ASSIGNMENT.
                    fseek(fid,4*size(handles.spikes,2),'cof');              %Skip the spike shape.
                    n = n + 1;              %Advance to the next spike.
                end
            end
        end
    end
...
%     fclose(fid);
%     clear data;     %Clear the data structure to free up RAM.
    if ~isempty(max_spike_break)    %If we've got more than 25,000 spikes...
        handles.spikes(max_spike_break:handles.numspikes,:) = [];   %Throw spikes when we have more than ~25,000.
        handles.cluster(max_spike_break:handles.numspikes) = [];
        handles.times(max_spike_break:handles.numspikes) = [];
        handles.numspikes = max_spike_break - 1;        %Set the number of spikeshapes we've grabbed.
    end
    handles = PlotSpikes(handles, hObject);         %Plot the spikeshapes in the main window
    handles = PlotClusters(handles, hObject);       %Plot the Templates, PSTHs, and ISIs.
    handles = PlotSecondary(handles, hObject);      %Plot the secondary plot depending on sort type.
...
function cmdSavePCA_Callback(hObject, eventdata, handles)
    ...
    temp = repmat(NaN,2*length(handles.draw),temp+1);   %Define a simple matrix to hold the data.
...
function cmdLoadPCA_Callback(hObject, eventdata, handles)
    ...
    if file ~= 0
        ...
        for i = 1:size(temp,1)/2            %Step through by cluster.
            ...
            handles.draw(i).values = temp(2*i-1:2*i,2:max(find(~isnan(temp(2*i,:)))));  %Grab the boundary values.
        end
...
function cmdSaveWindows_Callback(hObject, eventdata, handles)
    ...
    temp = repmat(NaN,temp,4*length(handles.windows));   %Define a simple matrix to hold the data.
...
function hscPointLimit_Callback(hObject, eventdata, handles)
