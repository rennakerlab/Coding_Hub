...
function [handles, max_spike_break] = LoadSpikes(handles, hObject, save_option)
    ...
    handles.spikes = [];            %Empty out the spikeshape matrix.
    handles.times = [];             %Empty out the spike time matrix.
    handles.cluster = [];           %Empty out the cluster matrix.
            ...
            if ~isempty(i)
                ...
                for j = 1:numsweeps
                    if ~feof(fid)
                        ...
                        for m = 1:numspikes
                            handles.times = [handles.times; single(fread(fid,1,'float32'))];    %Grab the spike time.
                            handles.cluster = [handles.cluster; uint8(fread(fid,1,'uint8'))];	%Grab the cluster assignment
                            handles.spikes = [handles.spikes; single(fread(fid,num_spike_samples,'float32')')];   %Grab the spike shape.
                        end
                        %If we've loaded up enough spikes for spike-sorting,
                        %stop loading.
                        if size(handles.spikes,1) > handles.max_spikes  %Check to see if we're at maximum...
                            max_spike_break = size(handles.spikes,1);   %If so, save the number of spikes.
...
function handles = SaveClusters(handles, hObject)
    ...
    if strcmpi(handles.sort_type,'Template')    %If we're template sorting...
        ...
        distances = repmat(NaN,1,handles.num_clusters);     %Set up a matrix to receive Euclidian distances.
    end
    numparams = fread(fid,1,'int8');         	%Number of stimulus parameters.
    for i = 1:numparams                         %Step through the number of parameters.
        numchar = fread(fid,1,'int16');      	%Number of characters in a parameter name.
...
%                                 handles.cluster(find(test == 1)) = i;   %Any spikeshapes that made it through all the windows are added to the cluster.
                        ...
                        if max_spike_break == 0
                            handles.spikes = [handles.spikes; spikeshape]; 	%Load up spikeshapes.
                            handles.cluster = [handles.cluster; cluster];	%Load up cluster assignments.
                            handles.times = [handles.times; spiketime];   	%Load up spike times.
                        end
                    end
