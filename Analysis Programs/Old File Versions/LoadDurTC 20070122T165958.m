...
for i = 1:length(stimuli)
    ...
    if isempty(find(stimuli(i,3) == durs))
        durs = [durs; stimuli(i,3)];
    end
end
...
for i = 1:length(data);
    ...
    for j = 1:numreps;
        ...
        try;
            temp(j,:) = histc(data(i).sweep(j).spikes,[onsets(i):(onsets(i) + sweeplength)]);
        catch;
...
%     [a,b] = size(strf(i).smooth);
    ...
    title([file(1:3) ': Intensity: ' num2str(durs(i)) ' ms']);
    a = colorbar;
