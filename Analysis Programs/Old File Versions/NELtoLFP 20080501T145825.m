...
for i = 1:length(data.stim)         %New Stimulus.
    ...
    for j = 1:size(data.stim(i).signal,1)       %New Sweep.
        if ~isnan(data.stim(i).signal(j,1));     %Continue only if this is a valid sweep.
            ...
            signal = [repmat(signal(1),1,500), signal, repmat(signal(length(signal)),1,500)];   %Add tails to prevent edge effects in the filter.
            signal = filtfilt(lowpass(1,:),lowpass(2,:),signal);        %Applying the lowpass filter.
            signal = filtfilt(highpass(1,:),highpass(2,:),signal);      %Applying the highpass filter.
            signal = signal(501:(length(signal)-500));                  %Cut off tails.
