        ...
        if ~isempty(i)
            ...
            for j = 1:numsweeps
                ...
                if numspikes == 0
                    ...
                    data.stim(i).spikes(sweep).times = single(zeros(numspikes,1));
                    data.stim(i).spikes(sweep).shapes = single(zeros(numspikes,num_spike_samples));
                    data.stim(i).spikes(sweep).cluster = uint8(zeros(numspikes,1));
                end
                for m = 1:numspikes
                    data.stim(i).spikes(sweep).times(m) = single(fread(fid,1,'float32'));       %Reading spike time.
                    data.stim(i).spikes(sweep).cluster(m) = uint8(fread(fid,1,'uint8'));        %Reading cluster assignment.
                    data.stim(i).spikes(sweep).shapes(m,:) = single(fread(fid,num_spike_samples,'float32')');	%Reading spike shape.
                end
            end
        end
    catch
