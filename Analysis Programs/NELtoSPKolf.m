...
%
%NELtoSPK.m - Rennaker Neural Engineering Lab, 2007
%
...
%
%   * 'Notch' - Apply a 60 Hz notch filter to the raw, unfiltered data
%               prior to any other processing, set with values of 'On' or
%               'Off'.  Default value is 'Off'.
%
%   Last updated January 20, 2010, by Drew Sloan.


%We'll keep track of how long it takes to threshold each file.
tic;
...
saveplots = 1;                  %Saving of representive threshold plots is turned off.
min_thresh = 3;                 %Minimum threshold for detection, in standard deviations of the noise.
high_pass_cutoff = 500;         %High-pass cutoff for the passband filter, in Hz.
...
notch = 0;                      %The extra 60-Hz notch filter is turned off by default.
...
for i = 1:2:length(varargin)
    if length(varargin) <= i    %Input arguments must come in pairs, the property name and it's specified value.
        ...
        elseif strcmpi(cell2mat(varargin(i)),'SetThreshold')    %Setting the 'SetThreshold' property.
            if isstr(cell2mat(varargin(i+1))) || ...     %'SetThreshold' input must be numeric and non-zero.
                ...
                if thresh > 0 && ~threshold_type_override     
                    threshold_type = 'POS';
                elseif ~threshold_type_override
                    threshold_type = 'NEG';
                end
        ...
        elseif strcmpi(cell2mat(varargin(i)),'Notch');	%Option to apply an extra 60-Hz notch filter
            if strcmpi(cell2mat(varargin(i+1)),'On')
                notch = 1;     %Turn displays on.
            elseif strcmpi(cell2mat(varargin(i+1)),'Off')
                notch = 0;     %Turn displays off.
            else    %If the input value is neither 'On' or 'Off', then indicate error.
                error('- The ''Notch'' property must set to either ''On'' or ''Off''.');
            end
...
%Here we'll generate the filter coefficients for the optional 60-Hz and 120-Hz notch filters.
[b,a] = butter(2,[55 65]*2/data.sampling_rate,'stop');       
notch_coefs = [b; a];
...
for i = 1:length(data.stim)
    for j = 1:size(data.stim(i).signal,1)
        ...
        if notch == 1                                                       %If the user's specified use of the 60-Hz notch filter...
            signal = filtfilt(notch_coefs(1,:),notch_coefs(2,:),...
                double(signal));                                            %Apply the 60-Hz notch filter.
        end
    ...
    for i = 1:length(data.stim)
        for j = 1:size(data.stim(i).signal,1)
            if strcmpi(thresh_method,'auto')                    	%If the threshold is not preset by the user, then auto-threshold.
                ...
                signal = signal(signal < noise_ceiling);          	%Excluding samples above the noise ceiling.
...
if saveplots
    ...
    for i = 1:size(a,1);
        ...
        if length(signal)/data.sampling_rate > sniplen  %If the signal is longer that the desired snippet length, we'll truncate.
            ...
            temp = ceil(rand*temp);    %Randomly picking one starting point.   
