...
SPC.max_clus = 5;               %Maximum number of clusters allowed.
...
for i = 1:2:length(varargin)
    if length(varargin) <= i    %Input arguments must come in pairs, the property name and it's specified value.
        ...
        elseif strcmpi(cell2mat(varargin(i)),'Display');    %Turn plot displays on or off.
            ...
            error(['- ''' cell2mat(varargin(i)) ''' is not a recognized input argument.']);
        end
    end
end
...
temp = find(file == '\',1,'last');                                          %Find the path in the file name if it's included.
if isempty(temp)    
...
all_spikes = single(zeros(SPC.max_spikes,num_spike_samples));	%preallocate a matrix to hold spikeshapes.
index = 1;                                                      %Keep track of the current row in the spikeshapes matrix.
            ...
            for j = 1:numsweeps
                if ~feof(fid)
                    ...
                    for m = 1:numspikes
                        ...
                        all_spikes(index,:) = single(fread(fid,num_spike_samples,'float32')');   %Grab the spike shape.
                        index = index + 1;
                    end
...
if index < SPC.max_spikes
    all_spikes(index:SPC.max_spikes,:) = [];
end
...
if SPC.min_clus < 5     %If there's very few sweeps, we'll arbitrarily set the minimum cluster size at 5 spikes.
    SPC.min_clus = 5;
end
...
if strcmpi(sort_feature,'WAV');         %If we're applying wavelet clustering...
    for i = 1:numspikes
        [c,l] = wavedec(all_spikes(i,:),scales,'haar'); %#ok<NASGU>
        all_spikes(i,:) = c(1:size(all_spikes,2));
    end
    trace = zeros(1,num_spike_samples);
...
if exist([workingpath 'cluster.exe'],'file') == 0                           %If cluster.exe isn't in the current directory.
    directory = which('cluster.exe');                                       %Find the full path for cluster.exe.
    copyfile(directory,workingpath);                                        %Copy cluster.exe to the working directory.
end
...
status = dos(['cluster.exe %s.run',SPC.fname]);   %Execute cluster.exe in DOS.

disp(status);

%We'll read back in the output of the SPC program.  If there weren't enough
%spikes to run the SPC, just skip all the following code.
...
if exist([SPC.fname '.dg_01.lab'],'file')
...
aux = zeros(size(tree,1)-1,size(tree,2)-4);
for i = 5:size(tree,2)
    aux(:,i-4) = diff(tree(:,i));   %Change in the nth cluster size.
end
...
if displayopt
    ...
    %Temperature plot;
    a = zeros(clust_inputs-1,clust_inputs-1);     %With so many projection plots, it's tricky to find the right subplot indices for a large section.
    for i = 1:(clust_inputs-1)
        a(i,:) = (1:(clust_inputs-1)) + (i - 1)*(clust_inputs-1);
    end
    ...
    %Spike shape plot.
    a = zeros(clust_inputs-1,clust_inputs-1);     %Again, with so many projection plots, it's tricky to find the right subplot indices for a large section.
    for i = 1:(clust_inputs-1)
        a(i,:) = (1:(clust_inputs-1)) + (i - 1)*(clust_inputs-1);
    end
