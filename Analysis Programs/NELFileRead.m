...
while ~feof(fid)
    ...
    if ~isempty(i) && i > 0
        ...
        if any(abs(temp) > Inf)      
            ...
            if feof(fid)
                warning('NEL:NELFileReadError',...
                    ['NEL read error: Abrupt file end on sweep #' num2str(i) ', likely memory shortage during recording, terminating data read.']);
                ...
                warning('NEL:NELFileReadError',['NEL read error: Corrupted sweep #' num2str(i) ', sweep will not be loaded.']);
            end
        ...
        elseif length(temp) < size(data.stim(i).signal,2)       %Report the truncated sweep.
            warning('NEL:NELFileReadError',['NEL read error: Truncated sweep #' num2str(i) ', sweep will not be loaded.']);
        end
    elseif i < 0
        ...
        warning('NEL:NELFileReadError','NEL file error: Negative sweep index, terminating data read.');
        break;
    end
end
fclose(fid);
